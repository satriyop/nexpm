...
test('site operations dashboard explains why a location is not done', function () {
    ...
    expect($payload['metrics']['total_sites'])->toBe(1)
        ...
        ->and($row['ai_prompt'])->toBe('Kenapa site JKT-001 belum selesai dan apa masalah utamanya?')
        ->and($row['ai_prompt'])->not->toContain('contextual_page_summary')
        ->and($row['ai_prompt'])->not->toContain('Root blocker')
        ->and($row['ai_prompt'])->not->toContain('Access blocked by site security gate.')
