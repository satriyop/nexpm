...
test('site operations dashboard explains why a location is not done', function () {
    ...
    expect($payload['metrics']['total_sites'])->toBe(1)
        ...
        ->and($row['ai_prompt'])->toContain('contextual_page_summary')
        ->and($row['ai_prompt'])->toContain('site_id '.$site->id)
        ->and($row['ai_prompt'])->toContain('Root blocker: construction_missing_wo')
        ->and($row['ai_prompt'])->toContain('Operational symptom: stalled_assignment')
        ->and($row['ai_prompt'])->toContain('Access blocked by site security gate.')
