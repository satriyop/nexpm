...
function applyFilters(): void {
    ...
    router.get(DrafterAssignmentActions.index().url, query, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
function getAssignment(assignments: any, type: string): any | null {
    if (!assignments) {
        return null;
    }
    return Array.from(assignments).find((a: any) => a?.activity_type === type) || null;
}
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-4 p-4">
        <div class="flex flex-col gap-1">
            ...
            <p class="text-sm text-muted-foreground">Grouped by site · Read only</p>
        </div>
        ...
        >
            <div
                class="grid flex-1 grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7"
            >
                ...
                <div class="grid gap-1.5">
                    <label class="text-xs font-medium text-muted-foreground">Machine Type</label>
                    <Select v-model="machineTypeId" @update:model-value="applyFilters">
                        <SelectTrigger class="w-full">
                            <SelectValue placeholder="All types" />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem :value="ALL">All types</SelectItem>
                            <SelectItem v-for="mt in machineTypes" :key="mt.id" :value="mt.id.toString()">
                                {{ mt.name }}
                            </SelectItem>
                        </SelectContent>
                    </Select>
                ...
                <div class="grid gap-1.5">
                    ...
                    <Select v-model="subcontractorId" @update:model-value="applyFilters">
                        ...
                        <SelectContent>
                            ...
                            <SelectItem v-for="sc in subcontractors" :key="sc.id" :value="sc.id.toString()">
                                {{ sc.name }}
                            </SelectItem>
                        </SelectContent>
                    </Select>
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <template v-for="col in activityColumns" :key="col.type">
                                <td class="px-4 py-3">
                                    <div v-if="site && site.assignments">
                                        <div
                                            v-for="assignment in [getAssignment(site.assignments, col.type)]"
                                            :key="assignment?.id ?? `${site.id}-${col.type}`"
                                        >
                                            <div
                                            ...
                                            >
                                                ...
                                                <span
                                                    v-if="
                                                        ...
                                                        daysStalled(assignment)! >= sla.stalled_days
                                                    "
                                                ...
                                                <span
                                                    v-else-if="
                                                        daysStalled(assignment) !== null &&
                                                        daysStalled(assignment)! >= sla.slow_days
                                                    "
                            ...
                            <td class="px-4 py-3 text-right">
                                <Button
                                    v-if="getAssignment(site.assignments, 'SURVEY')"
                                    as-child
                                    variant="outline"
                                    size="sm"
                                >
                                    <Link :href="DrafterAssignmentActions.show(getAssignment(site.assignments, 'SURVEY')).url">
                                        <Eye class="size-4" />
                                    </Link>
                                </Button>
                                <span v-else class="text-xs text-muted-foreground">—</span>
                        ...
                        <tr v-if="!sites || !sites.data || sites.data.length === 0">
                            <td colspan="9" class="px-4 py-12 text-center text-sm text-muted-foreground">
                                <div class="flex flex-col items-center gap-2 py-4">
                                    ...
                                    <p class="text-xs">Try adjusting your filters or check again later.</p>
                                </div>
                            </td>
                        </tr>
                    </tbody>
