...
const tabs: Tab[] = [
    {
        ...
        description: 'Verified survey assignments',
    },
    {
        key: 'BAST',
        label: 'BAST Report',
        description: 'Verified BAST assignments (EVCS & BSS)',
    },
    {
        key: 'DAILY',
        label: 'Daily Report',
        description: 'All verified assignments snapshot',
    },
    {
        key: 'HISTORY',
...
const activeAssignments = computed<Assignment[]>(() => {
    ...
    switch (activeTab.value) {
        case 'SSR':
            list = props.ssrAssignments;
            break;
        case 'BAST':
            list = props.bastAssignments;
            break;
        case 'DAILY':
            list = [...props.ssrAssignments, ...props.bastAssignments];
            break;
    }

    if (activeTab.value === 'BAST' && siteTypeFilter.value !== 'ALL') {
        list = list.filter(
            (a) => a.site?.site_type?.name === siteTypeFilter.value,
        );
    }
...
watch(activeAssignments, () => {
    currentPage.value = 1;
});

const totalPages = computed(() =>
    Math.ceil(activeAssignments.value.length / PAGE_SIZE),
);
...
const pageEnd = computed(() =>
    Math.min(currentPage.value * PAGE_SIZE, activeAssignments.value.length),
);
...
const allSelected = computed(
    () =>
        ...
        selectedIds.value.length === activeAssignments.value.length,
);
...
function toggleAll(): void {
    selectedIds.value = allSelected.value
        ? []
        : activeAssignments.value.map((a) => a.id);
}
...
function regenerateReport(report: ReportRow): void {
    ...
    router.post(
        AdminReportActions.regenerate(report).url,
        {},
        {
            onFinish: () => {
    ...
    <div class="flex h-full flex-1 flex-col gap-4 p-4">
        ...
        <div class="flex flex-col gap-1">
            ...
            <p class="text-sm text-muted-foreground">
                Select verified assignments and generate reports (SSR → PDF,
                BAST/Daily → Excel)
            </p>
        </div>
            ...
            >
                <div class="flex flex-col gap-1.5">
                    ...
                    <p class="text-xs text-muted-foreground">
                        {{ activeAssignments.length }} available ·
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        <tr
                            ...
                            :class="{
                                'bg-muted/20': selectedIds.includes(
                                    assignment.id,
                                ),
                            }"
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-col gap-1">
                                    ...
                                    <span
                                        ...
                                        "
                                        class="inline-flex w-fit items-center rounded px-1.5 py-0.5 text-[10px] font-semibold tracking-wide uppercase"
                                        :class="
                        ...
                        <tr v-if="activeAssignments.length === 0">
                            ...
                            >
                                ...
                                >
                                    ...
                                    />
                                    <p class="font-medium">
                                        No verified assignments
                                    </p>
                                    <p class="text-xs">
                                        Assignments must be verified before they
                                        appear here.
                                    </p>
                                </div>
                            </td>
        ...
        >
            ...
            >
                ...
                <p class="text-xs text-muted-foreground">
                    Last 15 generated reports · click a row to see its sites
                </p>
            </div>
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            >
                                <td class="px-4 py-3">
                                    <span
                                        class="inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-semibold tracking-wide uppercase"
                                        :class="
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        ...
                                        >
                                            ...
                                            <span
                                                ...
                                                >{{ site.location_name }}</span
                                            >
                                            <span
                                                class="rounded bg-muted px-1 text-[10px] text-muted-foreground uppercase"
                                                >{{ site.activity_type }}</span
                                            >
                                        </span>
                                        <span
                                            v-if="report.sites.length === 0"
                                            class="text-xs text-muted-foreground"
                                            >No sites found.</span
                                        >
                                    </div>
