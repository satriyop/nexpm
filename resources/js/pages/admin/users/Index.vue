...
interface Subcontractor { id: number; name: string; main_contractor_id: number }
interface User {
    id: number;
    name: string;
    email: string;
...
const form = useForm({ name: '', email: '', password: '', role: '', main_contractor_id: '', subcontractor_id: '' });
...
<template>
    ...
    <div class="space-y-6 p-6">
        <div class="flex items-center justify-between">
            ...
            <Button @click="open = true"><Plus class="mr-1.5 h-4 w-4" />Add User</Button>
        </div>
                    ...
                    <tbody class="divide-y">
                        <tr v-for="user in users.data" :key="user.id" class="hover:bg-muted/30">
                            ...
                            <td class="px-4 py-3 text-muted-foreground">
                                {{ user.main_contractor?.name ?? user.subcontractor?.name ?? '—' }}
                            </td>
                        </tr>
        ...
        <DialogContent class="max-w-lg">
            ...
            <form class="space-y-4" @submit.prevent="submit">
                <div class="grid gap-1.5">
                    ...
                    <Input v-model="form.name" placeholder="Full name" autofocus />
                ...
                <div v-if="form.role === 'subcontractor'" class="grid gap-1.5">
                    ...
                    <Select v-model="form.subcontractor_id">
                        ...
                        <SelectContent>
                            <SelectItem v-for="sc in subcontractors" :key="sc.id" :value="String(sc.id)">{{ sc.name }}</SelectItem>
                        </SelectContent>
                    </Select>
