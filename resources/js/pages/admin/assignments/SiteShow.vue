...
import { AlertTriangle, ArrowLeft, ExternalLink, LockKeyhole, MapPin, Trash2 } from 'lucide-vue-next';
...
const woForm = useForm({
    cons_wo_number: constructionAssignment.value?.construction_data?.cons_wo_number ?? '',
});

function submitWo(): void {
    if (!constructionAssignment.value) { return; }
    router.visit(
        AdminAssignmentActions.updateConstructionPrerequisite(constructionAssignment.value.id).url,
        {
            method: 'patch',
        ...
        <div class="flex flex-col gap-2">
            ...
            <div class="flex flex-col gap-2">
                <div class="flex items-center gap-2 text-2xl font-semibold">
                    ...
                    <span class="font-normal text-muted-foreground">{{ site.location_name }}</span>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                    <span v-if="locationParts" class="flex items-center gap-1 text-sm text-muted-foreground">
                        <MapPin class="size-3.5" />
                        {{ locationParts }}
                    </span>
                    <Badge v-if="site.site_type" variant="outline" class="text-xs">
                        {{ site.site_type.name }}
                    </Badge>
                </div>
            </div>
        ...
        <Card>
            <CardContent class="grid gap-4 pt-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <p class="font-medium">{{ site.project?.client?.name ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-xs text-muted-foreground">Main Contractor</p>
                    <p class="font-medium">{{ site.project?.main_contractor?.name ?? '—' }}</p>
                </div>
                ...
                <CardContent class="pb-3">
                    <div v-if="surveyAssignment" class="flex flex-col gap-3">
                        ...
                        >
                            ...
                            <AlertDescription class="text-xs text-amber-900/90 dark:text-amber-100/90">
